...
namespace MyAssessment.Controllers
{
    ...
    public class ApiController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddMapel([FromBody] MapelItem mapel)
        {
            try
            {
                if (string.IsNullOrEmpty(mapel.Nama) || string.IsNullOrEmpty(mapel.Kode))
                {
                    return BadRequest(new { success = false, message = "Nama dan Kode Mapel wajib diisi" });
                }

                // Cek duplikat kode
                var existing = await _db.MapelList.Find(m => m.Kode == mapel.Kode).FirstOrDefaultAsync();
                if (existing != null)
                {
                    return BadRequest(new { success = false, message = "Kode Mapel sudah digunakan" });
                }

                mapel.Id = null; // Biarkan MongoDB generate ObjectId
                await _db.MapelList.InsertOneAsync(mapel);
                return Ok(new { success = true, message = "Mapel berhasil ditambahkan", data = mapel });
            }
            catch (Exception ex)
            {
                return BadRequest(new { success = false, message = "Gagal menambahkan mapel: " + ex.Message });
            }
        }
        ...
        public async Task<IActionResult> UpdateMapel(string id, [FromBody] MapelItem mapel)
        {
            try
            {
                ...
                var existing = await _db.MapelList.Find(filter).FirstOrDefaultAsync();
                if (existing == null)
                {
                    return NotFound(new { success = false, message = "Mapel tidak ditemukan" });
                }

                // Cek duplikat kode (kecuali diri sendiri)
                var duplicate = await _db.MapelList.Find(m => m.Kode == mapel.Kode && m.Id != id).FirstOrDefaultAsync();
                if (duplicate != null)
                {
                    return BadRequest(new { success = false, message = "Kode Mapel sudah digunakan" });
                }

                var update = Builders<MapelItem>.Update
                    .Set(m => m.Nama, mapel.Nama)
                    .Set(m => m.Kode, mapel.Kode)
                    .Set(m => m.Kelompok, mapel.Kelompok)
                    .Set(m => m.KKM, mapel.KKM);

                await _db.MapelList.UpdateOneAsync(filter, update);
                var updated = await _db.MapelList.Find(filter).FirstOrDefaultAsync();
                return Ok(new { success = true, message = "Mapel berhasil diperbarui", data = updated });
            }
            catch (Exception ex)
            {
                return BadRequest(new { success = false, message = "Gagal memperbarui mapel: " + ex.Message });
            }
        }
        ...
        public async Task<IActionResult> DeleteMapel(string id)
        {
            try
            {
                ...
                // Cek apakah mapel sudah memiliki data nilai
                var hasGrades = await _db.Grades.Find(g => g.Mapel == existing.Nama).FirstOrDefaultAsync();
                if (hasGrades != null)
                {
                    return BadRequest(new { success = false, message = "Mapel tidak dapat dihapus karena sudah memiliki data nilai" });
                }

                // Cek apakah mapel sudah memiliki KD
                var hasKD = await _db.Competencies.Find(k => k.Mapel == existing.Nama).FirstOrDefaultAsync();
                if (hasKD != null)
                {
                    return BadRequest(new { success = false, message = "Mapel tidak dapat dihapus karena sudah memiliki data Kompetensi Dasar" });
                }
            ...
            {
                return BadRequest(new { success = false, message = "Gagal menghapus mapel: " + ex.Message });
            }
        }
